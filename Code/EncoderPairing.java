...
public class EncoderPairing {
	public static Map<String, String> compress(String[] number) {   
	    ...
	    for(int i = 0; i < 10; i++) {
	    	...
	    	for (String s : freq.keySet())   
	            pq.add(new PairNode(s, freq.get(s), null, null));  
	  
	    	// merge two smallest trees  
	    	while (pq.size() > 1) {  
	    		PairNode left  = pq.poll();  
	    		PairNode right = pq.poll();  
	    		PairNode parent = new PairNode("", left.freq + right.freq, left, right);  
	    		pq.add(parent);  
	    	}  
	    	// stop counting time
	...
	public static void buildCode(Map<String, String> codetable, PairNode x, String s) {  
	    if (!x.isLeaf()) {  
